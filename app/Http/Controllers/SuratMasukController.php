...
class SuratMasukController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validateData = $request->validate([
            ...
            'file'=> 'required|file|mimes:pdf|max:2048', // max dalam kilobytes (2048 KB = 2 MB)
        ]);

        try {
            $validateData['type'] = 'surat_masuk';
            $checkData = Letters::where("no_unit", $request->no_unit)->where("type", "surat_masuk")->where("nama_unit", $request->nama_unit)->where("no_berkas", $request->no_berkas)->first();
            if ($checkData) {
            ...
            if ($request->hasFile('file')) {
                ...
                $folder = "surat_masuk/{$request->nama_unit}";
                $file->storeAs("public/{$folder}", $filename);

                $validateData['file'] = "surat_masuk"."/".$request->nama_unit."/". "{$filename}";
            }
    ...
    public function edit(Request $request)
    {
        ...
        try {
            ...
            $validateData['type'] = 'surat_masuk';
            $checkData = Letters::where("no_unit", $request->no_unit)->where("type", "surat_masuk")->where("nama_unit", $request->nama_unit)->where("no_berkas", $request->no_berkas)->where("id", "!=" , $request->id)->first();
            if ($checkData) {
    ...
    {
        $data = [
            ['jenis_berkas' => 'Surat', 'no_berkas' => '001', 'nama_unit' => 'Unit A', 'no_unit' => 'U001', 'tanggal_masuk' => '2025-05-17'],
            ['jenis_berkas' => 'Dokumen', 'no_berkas' => '002', 'nama_unit' => 'Unit B', 'no_unit' => 'U002', 'tanggal_masuk' => '2025-05-16'],
            ['jenis_berkas' => 'Laporan', 'no_berkas' => '003', 'nama_unit' => 'Unit C', 'no_unit' => 'U003', 'tanggal_masuk' => '2025-05-15'],
        ];

        // Header CSV
        $csvHeader = ['jenis Berkas', 'No Berkas', 'Nama Unit', 'No Unit', 'Tanggal Masuk'];
    ...
    {
        $request->validate([
            'file_surat_masuk' => 'required|mimes:csv,txt|max:2048',
        ]);
        ...
        foreach ($rows as $index => $row) {
            ...
            $validator->after(function ($validator) use ($request, $data, $arrayJenisBerkas) {
                ...
                $tanggal = $data["Tanggal Masuk"] ?? null;
            ...
            $dataOutMails[] = [
                ...
                "tanggal_masuk" => $data["Tanggal Masuk"],
            ];
        }
        DB::beginTransaction();
        ...
        try {
            foreach ($dataOutMails as $item) {
                $existingLetter = Letters::where('type', 'surat_masuk')
            ...
            return back()->with('toast_success', "Success Upload Dokumen Masuk");
        } catch (\Throwable $th) {
            DB::rollBack();
            return back()->with('toast_error', $th->getMessage());
        }
